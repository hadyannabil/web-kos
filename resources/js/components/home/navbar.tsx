    ...
    return (
        <header className="sticky top-0 z-50 border-b border-[#DDE8D8]/80 bg-white/95 backdrop-blur">
            <div className="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12">
                <Link href="/" aria-label="KosKita Residence - Beranda">
                    <BrandLogo />
                </Link>
                ...
                >
                    {navLinks.map((link) => (
                        <a
                            key={link.href}
                            href={link.href}
                            className="text-sm font-semibold text-[#1F2A24] transition hover:text-[#4F6F52] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#4F6F52]"
                        >
                            {link.label}
                        </a>
                    ))}
                ...
                <button
                    type="button"
                    className="grid size-11 place-items-center rounded-xl text-[#4F6F52] transition hover:bg-[#F3F7F1] lg:hidden"
                    aria-label={
            ...
            {isOpen && (
                <nav
                    ...
                    className="border-t border-[#DDE8D8] bg-white px-5 py-5 shadow-xl lg:hidden"
                    ...
                    <div className="mx-auto flex max-w-2xl flex-col gap-1">
                        {navLinks.map((link) => (
                            <a
                                key={link.href}
                                href={link.href}
                                className="rounded-xl px-4 py-3 text-sm font-semibold text-[#1F2A24] hover:bg-[#F3F7F1] hover:text-[#4F6F52]"
                                onClick={closeMenu}
                            >
                                {link.label}
                            </a>
